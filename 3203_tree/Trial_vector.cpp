...
public:
    ...
    void readInTree(const vector<vector<int>> edges, vector<Node>* Tree){
    ...
    int shortestMaxLen( const vector<Node>& Tree , int *maxDiameter){
        if(Tree.empty()) return 0;
        int min = INT_MAX;
        int temp;
        for(int i=0; i<Tree.size(); i++){ 
            temp = maxLen(Tree, i, Tree[i], -1);
    ...
    // BFS in essence
    int maxLen(const vector<Node>& Tree,int rootIdx, Node root, int parent){
        int max = -1;
        int temp;
    ...
    int minimumDiameterAfterMerge(vector<vector<int>>& edges1, vector<vector<int>>& edges2) {
        vector<Node> Tree1(edges1.size() + 1);
        vector<Node> Tree2(edges2.size() + 1);
